...
if len(sys.argv) == 4:
    ...
    if os.path.isfile(sys.argv[2]):
        path = sys.argv[2]
        subprocess.run("cp {} script_modified.asm".format(path), shell=True)
        if sys.argv[3].endswith("A"):
            subprocess.run("sed -i 's/0x41/0x42/' script_modified.asm", shell=True)
            subprocess.run("sed -i 's/A/B/' script_modified.asm", shell=True)
        subprocess.run("sed -i 's/rdx+5/rdx+{}/' script_modified.asm".format(pathParameterLength), shell=True)
        subprocess.run("sed -i 's#ls -l#{}#' script_modified.asm".format(pathParameter), shell=True)
        subprocess.run(" nasm -f bin script_modified.asm -o a.bin", shell=True)
    ...
    print("Wie folgt zu Benutzen:", "Programmname:", sys.argv[0], "Bufferadresse",
          "Assemblerprogramm", "'Linux Bash Befehl'")
    sys.exit(0)
